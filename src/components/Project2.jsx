    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">Project Loading...</h2>
          <p className="text-gray-600">If this persists, there may be an issue with the project data.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="h-full bg-white text-gray-900 overflow-y-auto">
      ...
      <div className="max-w-6xl mx-auto px-8">
        ...
        <SectionLayout title="Project Goals">
          <div className="space-y-8">
            {project.projectGoals.map((goal, index) => (
              <div key={index}>
                <span className="font-bold text-lg text-gray-900 block mb-1">{goal.title}</span>
                <span className="text-gray-800 text-base font-normal block">{goal.description}</span>
              </div>
            ))}
          </div>
        </SectionLayout>
        ...
        <TwoImageGrid leftImage={project.logoBoilit} rightImage={project.uiMockups} leftAlt="Logo" rightAlt="UI Mockups" />

        {/* SECTION 7: User Research Section - Uses project.userResearch */}
        <SectionLayout title="User Research">
          <div className="space-y-8">
            <div>
              <span className="font-bold text-lg text-gray-900 block mb-4">{project.userResearch.whoAreUsers.title}</span>
              <p className="text-gray-800 text-base font-normal mb-4">
                {project.userResearch.whoAreUsers.description}
              </p>
              <p className="text-gray-800 text-base font-normal mb-2">i found some Common problems included:</p>
              <ul className="list-disc pl-6 text-gray-800 text-base font-normal mb-4">
                {project.userResearch.whoAreUsers.problems.map((problem, index) => (
                  <li key={index}>{problem}</li>
                ))}
              </ul>
            </div>
            <div>
              <span className="font-bold text-lg text-gray-900 block mb-4">{project.userResearch.mainPersona.title}</span>
        ...
        <section className="py-20 flex flex-col items-left">
          <div className="flex mb-8">
            <button
              className={`px-8 py-4 text-lg font-medium rounded-tl-lg rounded-bl-lg border border-black border-r-0 focus:outline-none transition-colors duration-200 ${selectedJourney === 'before' ? 'bg-black text-white' : 'bg-white text-black'}`}
              onClick={() => setSelectedJourney('before')}
            >
              Before - User journey
            </button>
            <button
              className={`px-8 py-4 text-lg font-medium rounded-tr-lg rounded-br-lg border border-black focus:outline-none transition-colors duration-200 ${selectedJourney === 'after' ? 'bg-black text-white' : 'bg-white text-black'}`}
        ...
        <SectionLayout title="Design Solution">
          <div className="space-y-4">
            <span className="font-bold text-lg text-gray-900 block mb-1">{project.designSolution.title}</span>
            <span className="text-gray-800 text-base font-normal block">{project.designSolution.description}</span>
          </div>
        </SectionLayout>
        ...
        <div className="my-8" style={{ width: '68%', margin: 'auto' }}>
          <span className="font-bold text-lg text-gray-900 block mb-1">{project.israeliDesign.title}</span>
          <span className="text-gray-800 text-base font-normal block">{project.israeliDesign.description}</span>
        </div>
        ...
        <section className="mb-20">
          <GalleryGrid
            images={project.sketches ? project.sketches.map((src, i) => ({ src, alt: `Sketch ${i + 1}` })) : []}
            columns={3}
            aspect="aspect-[3/4]"
            gap="gap-8"
          />
