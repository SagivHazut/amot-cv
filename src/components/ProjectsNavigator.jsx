            ...
            .map((project) => (
              ...
              >
                <div className="w-62 h-52 bg-gray-100 overflow-hidden mb-3 shadow-sm relative group">
                  ...
                  />
                  {/* Title overlay */}
                  <div className="absolute top-2 left-2 bg-black bg-opacity-70 text-white text-sm font-semibold px-3 py-1 transition-opacity duration-300 opacity-0 group-hover:opacity-100">
                    {project.title}
                  </div>
                  {/* Tags overlay */}
                  <div className="absolute bottom-2 left-2 flex flex-wrap gap-2 transition-opacity duration-300 opacity-0 group-hover:opacity-100">
                    {project.tags && project.tags.map((tag, idx) => (
                      <span key={idx} className="bg-white bg-opacity-80 text-gray-800 text-xs px-2 py-0.5">
                        {tag.text}
                      </span>
                    ))}
                  </div>
